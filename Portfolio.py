...
if page == "Home":
    ...
    st.image("promesse.jpg", width=150, caption="Default Profile Picture")
    ...
    if os.path.exists(profile_data["resume_path"]):
        ...
        st.download_button(label="📄 Download Resume", data=resume_bytes, file_name="promesse.pdf", mime="application/pdf")
    else:
...
# Projects section
    ...
    with st.expander("📊 online shopping system"):
    ...
    with st.expander("🤖 online tranking system"):
        st.write("Developed  using PHP And Database")
       

    with st.expander("🌐 mobile app and web app for managing digital wardrobes"):
        st.write("Developing Using python,JavaScript,php,html,css ")
...
# Skills section
    ...
    skill_python = st.slider("Python", 5, 80, 90)
    st.progress(skill_python)

    skill_js = st.slider("JavaScript", 6, 70, 75)
    ...
    st.write("✔ software Engineering A2")
...
# Contact section
    ...
    with st.form("contact_form"):
        ...
        if submitted:
            st.success("✅ Message  successfully")

    st.write("📧 Email: igisubizomariep50@gmail.com")
    st.write("[🔗 )")
    st.write("[📂 https://github.com/promesse123/Car-Accessories.git")
    st.write(" Phone number: 0790783336")
